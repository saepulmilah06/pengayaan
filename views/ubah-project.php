...
if (isset($_POST['simpan'])) {
  ...
  if(!empty($foto)) {
    ...
    "update project set nama_project='$nama', keterangan='$keterangan'
    where id_project=$id"
    );
    ...
    if(move_uploaded_file($_FILES['foto']['tmp_name'], $folder)) {
      ...
      $q = mysqli_query($koneksi, 
      "update project set nama_project='$nama', keterangan='$keterangan', foto='$namaFoto'
      where id_project=$id"
      );
      $message = "<div class='alert alert-success'>Project berhasil diubah!</div>";
    }
...
<div class="row">
  <div class="col-md-6">
    <h1>
      Ubah Project <?=$data['nama_project']?>
    </h1>
    <?=@$message?>
    <form action="" method="post" enctype="multipart/form-data">
      <div class="form-group">
        <label>NAMA</label>
        <input type="text" name="nama" class="form-control" value="<?=$data['nama_project']?>" required>
      </div>
      <div class="form-group">
